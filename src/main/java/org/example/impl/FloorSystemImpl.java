...
public class FloorSystemImpl implements FloorSystem {
    Random random;
    public Floor[] floors;
    ...
    public FloorSystemImpl(int floorCount, long seed){
        this.random = new Random(seed);
        this.floorCount = floorCount;
        initializeFloors();
    }
    public FloorSystemImpl(int floorCount){
    ...
    public void initializeFloors(){
        this.floors = new FloorImpl[floorCount];
    ...
    public void addPersonToFloor(int floorID, int desiredFloorID){
        PersonImpl Gregory = new PersonImpl(allTimePeopleCount, this.floors[floorID], this.floors[desiredFloorID]);
        allTimePeopleCount++;
    ...
    public int[] status(int FloorID){
        return new int[]{floors[FloorID].getPeople().size(), floors[FloorID].isUpPressed() ? 1 : 0,
                floors[FloorID].isDownPressed() ? 1 : 0};
    }
    ...
    public Floor[] addFloorToEnd(Floor[] array) {
        ...
        newArray[newArray.length - 1] = new FloorImpl(newArray.length - 1);
    ...
    public void decrementFloorCount(){
        ...
        for (Floor floor :
            ...
            for (Person person :
                    floor.getPeople()) {
                if (person.getDesiredFloor().getID() >= floorCount) {
                    person.setDesiredFloor(floors[floorCount - 1]);
                }
            }
        }
    }
